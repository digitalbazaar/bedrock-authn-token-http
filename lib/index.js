...
import * as schemas from '../schemas/bedrock-authn-token-http.js';
import {asyncHandler} from '@bedrock/express';
import {createValidateMiddleware} from '@bedrock/validation';
import crypto from 'node:crypto';
import {generateId} from 'bnid';
import {MultifactorStrategy} from './MultifactorStrategy.js';
...
bedrock.events.on('bedrock-express.configure.routes', app => {
    ...
    asyncHandler(async (req, res) => {
      ...
      if(type === 'nonce') {
        ...
        await brAuthnToken.set({
          ...
          authenticationOrigin: _getOrigin({req}),
          authenticationMethod, requiredAuthenticationMethods, typeOptions
        });
        res.status(204).end();
      } else {
  ...
  app.get(
    typeRoute + '/hash-parameters',
    // Note: authentication not required for obtaining hash params for a user
    ...
    asyncHandler(async (req, res) => {
      ...
      // if a token does not exist `brAuthnToken.get`, return fake token
      // hash parameters; but to reduce time differences in code branching,
      // always compute and do it in parallel with the database hit and
      // a jitter delay
      const delay = Math.random() * cfg.fakeTokenOptions.jitter;
      const getTokenPromise = Promise.all([
        brAuthnToken.get({accountId, email, type}),
        setTimeout(delay)
      ]).then(([token]) => token, e => e);

      // generate fake token while we wait for database and jitter
      ...
      let token = await getTokenPromise;
      if(token instanceof Error) {
        // throw any errors other than not found
        if(token.name !== 'NotFoundError') {
          throw token;
        }
        token = fakeToken;
      }
      const {hashParameters} = token;
      res.json({hashParameters});
    }));
  ...
  app.get(
    ...
    createValidateMiddleware(
      {querySchema: schemas.getClientRegistrationQuery()}),
    asyncHandler(async (req, res) => {
...
function _getOrigin({req}) {
  const {
    allowMissingOrigin, authenticationOriginAllowList
  } = config['authn-token-http'];
  const origin = req.get('origin');
  // a supplied origin must be allow-listed; a missing origin is only allowed
  // if configured
  if(origin === undefined) {
    // no origin implies a non-browser client; return none so a consumer falls
    // back to its own branding
    if(allowMissingOrigin) {
      return;
    }
    throw new BedrockError(
      'The request must include an origin.',
      'NotAllowedError', {
        httpStatusCode: 403,
        public: true
      });
  }
  if(!authenticationOriginAllowList.includes(origin)) {
    throw new BedrockError(
      `The request origin "${origin}" is not allowed.`,
      'NotAllowedError', {
        httpStatusCode: 403,
        public: true
      });
